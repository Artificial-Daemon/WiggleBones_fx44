bl_info = {
    "name": "WiggleBones 4.4",
    "author": "Steve Miller, Artificial_Daemon",
    "version": (2, 2, 5),
    "blender": (4, 4, 0),
...
def collide(b,dg,head=False):
    # Pass-through cooldown: skip collision while active
    try:
        if getattr(b, 'wiggle_jam_mode', 'HARD_STOP') == 'PASS_THROUGH':
            if getattr(b, 'wiggle_pass_cooldown', 0) > 0:
                b.wiggle_pass_cooldown -= 1
                if head:
                    b.wiggle.collision_ob_head = None
                    b.wiggle.collision_normal_head = Vector((0,0,0))
                else:
    ...
    elif collider_type == 'Objects':
        items = b.wiggle_collider_multi_head if head else b.wiggle_collider_multi
        for it in items:
            if it.obj and it.obj.type == 'MESH' and it.obj.name in bpy.context.scene.objects:
                colliders.append(it.obj)
...
#                sy = sy*(b.parent.length/b.parent.bone.length)
    ...
    # Clamp stretch to avoid bone scaling artifacts
    try:
        preserve = getattr(b, 'wiggle_preserve_length', True)
        max_stretch = getattr(b, 'wiggle_max_stretch', 0.15)
        max_compress = getattr(b, 'wiggle_max_compress', 0.0)
        jam_mode = getattr(b, 'wiggle_jam_mode', 'HARD_STOP')
    except Exception:
        preserve, max_stretch, max_compress, jam_mode = True, 0.15, 0.0, 'HARD_STOP'
    if preserve:
        hi = 1.0 + max(0.0, max_stretch)
        lo = 1.0 - max(0.0, max_compress)
        # detect conditions before clamping for behavior decisions
        over_stretch = sy > hi + 1e-6
        over_compress = sy < lo - 1e-6
        # clamp
        if sy > hi:
            sy = hi
        if sy < lo:
        ...
        # behavior
        if over_stretch:
            if jam_mode == 'HARD_STOP':
                try:
                    b.wiggle.velocity *= 0.2
                except Exception:
                    pass
            else:  # PASS_THROUGH
                # trigger cooldown for a few frames so collide() ignores collisions
                try:
                    b.wiggle_pass_cooldown = max(getattr(b, 'wiggle_pass_cooldown', 0), 3)
                except Exception:
                    pass
        if over_compress:
            # push tail outward a bit by easing velocity along the bone axis
            try:
                b.wiggle.velocity *= 0.85
            except Exception:
                pass
...
#        return context.active_pose_bone and not context.active_pose_bone.bone.use_connect
    ...
    def draw(self,context):
        ...
        if b.wiggle_collider_type_head == 'Object':
            row = col.row(align=True)
            row.prop(b, 'wiggle_collider_head', text=' ')
            if b.wiggle_collider_head:
                if b.wiggle_collider_head.name in context.scene.objects:
                    collision = True
                else:
                    row.label(text='',icon='UNLINKED')
        elif b.wiggle_collider_type_head == 'Objects':
            row = col.row(align=True)
            row.prop(b, 'wiggle_collider_pick_head', text='Pick')
            ops = col.row(align=True)
            ops.operator('wiggle.cmulti_add_picked', text='Add Picked').head=True
            ops.operator('wiggle.cmulti_add_active', text='Add Active').head=True
            row = col.row(align=True)
            row.template_list('WIGGLE_UL_ColliderObjects','head', b, 'wiggle_collider_multi_head', b, 'wiggle_collider_multi_index_head', rows=3)
            row = col.row(align=True)
            row.operator('wiggle.cmulti_remove', text='Remove').head=True
            row.operator('wiggle.cmulti_clear', text='Clear').head=True
            collision = len(b.wiggle_collider_multi_head) > 0
        else:
            row = col.row(align=True)
        ...
        if collision:
            ...
            col.separator()
            col.prop(b,'wiggle_preserve_length')
            col.prop(b,'wiggle_max_stretch')
            col.prop(b,'wiggle_jam_mode')
            col.prop(b,'wiggle_max_compress')
            col.prop(b,'wiggle_max_compress')
...
#        return context.active_pose_bone
    ...
    def draw(self,context):
        ...
        if b.wiggle_collider_type == 'Object':
            row = col.row(align=True)
            row.prop(b, 'wiggle_collider', text=' ')
            if b.wiggle_collider:
                if b.wiggle_collider.name in context.scene.objects:
                    collision = True
                else:
                    row.label(text='',icon='UNLINKED')
        elif b.wiggle_collider_type == 'Objects':
            row = col.row(align=True)
            row.prop(b, 'wiggle_collider_pick', text='Pick')
            ops = col.row(align=True)
            ops.operator('wiggle.cmulti_add_picked', text='Add Picked').head=False
            ops.operator('wiggle.cmulti_add_active', text='Add Active').head=False
            row = col.row(align=True)
            row.template_list('WIGGLE_UL_ColliderObjects','tail', b, 'wiggle_collider_multi', b, 'wiggle_collider_multi_index', rows=3)
            row = col.row(align=True)
            row.operator('wiggle.cmulti_remove', text='Remove').head=False
            row.operator('wiggle.cmulti_clear', text='Clear').head=False
            collision = len(b.wiggle_collider_multi) > 0
        else:
            row = col.row(align=True)
...
class WiggleColliderItem(bpy.types.PropertyGroup):
    obj: bpy.props.PointerProperty(name='Object', type=bpy.types.Object, poll=collider_poll)

class WIGGLE_UL_ColliderObjects(bpy.types.UIList):
    bl_idname = 'WIGGLE_UL_ColliderObjects'
    def draw_item(self, context, layout, data, item, icon, active_data, active_propname, index):
        layout.prop(item, 'obj', text='')

class WIGGLE_OT_CMultiAddActive(bpy.types.Operator):
    bl_idname = 'wiggle.cmulti_add_active'
    bl_label = 'Add Active'
    head: bpy.props.BoolProperty(default=False)
    @classmethod
    def poll(cls, context):
    ...
    def execute(self, context):
        ...
        lst = b.wiggle_collider_multi_head if self.head else b.wiggle_collider_multi
        if any(it.obj == ob for it in lst): return {'CANCELLED'}
        it = lst.add(); it.obj = ob
        if self.head: b.wiggle_collider_multi_index_head = len(lst)-1
        else: b.wiggle_collider_multi_index = len(lst)-1
        return {'FINISHED'}

class WIGGLE_OT_CMultiAddPicked(bpy.types.Operator):
    bl_idname = 'wiggle.cmulti_add_picked'
    bl_label = 'Add Picked'
    head: bpy.props.BoolProperty(default=False)
    def execute(self, context):
        b = context.active_pose_bone
        if not b: return {'CANCELLED'}
        pick = b.wiggle_collider_pick_head if self.head else b.wiggle_collider_pick
        if not pick or pick.type != 'MESH': return {'CANCELLED'}
        lst = b.wiggle_collider_multi_head if self.head else b.wiggle_collider_multi
        if any(it.obj == pick for it in lst): return {'CANCELLED'}
        it = lst.add(); it.obj = pick
        if self.head: b.wiggle_collider_multi_index_head = len(lst)-1
        else: b.wiggle_collider_multi_index = len(lst)-1
        return {'FINISHED'}

class WIGGLE_OT_CMultiRemove(bpy.types.Operator):
    bl_idname = 'wiggle.cmulti_remove'
    bl_label = 'Remove'
    head: bpy.props.BoolProperty(default=False)
    def execute(self, context):
        b = context.active_pose_bone
        if not b: return {'CANCELLED'}
        if self.head:
            idx = b.wiggle_collider_multi_index_head; lst = b.wiggle_collider_multi_head
        else:
            idx = b.wiggle_collider_multi_index; lst = b.wiggle_collider_multi
        if 0 <= idx < len(lst):
            lst.remove(idx)
            if self.head: b.wiggle_collider_multi_index_head = min(max(0, idx-1), len(lst)-1)
            else: b.wiggle_collider_multi_index = min(max(0, idx-1), len(lst)-1)
            return {'FINISHED'}
        return {'CANCELLED'}

class WIGGLE_OT_CMultiClear(bpy.types.Operator):
    bl_idname = 'wiggle.cmulti_clear'
    bl_label = 'Clear'
    head: bpy.props.BoolProperty(default=False)
    def execute(self, context):
        b = context.active_pose_bone
        if not b: return {'CANCELLED'}
        if self.head:
            b.wiggle_collider_multi_head.clear(); b.wiggle_collider_multi_index_head = 0
        else:
            b.wiggle_collider_multi.clear(); b.wiggle_collider_multi_index = 0
        return {'FINISHED'}


def register():

    # Multi-object collider support
    bpy.utils.register_class(WiggleColliderItem)
    bpy.utils.register_class(WIGGLE_UL_ColliderObjects)
    bpy.utils.register_class(WIGGLE_OT_CMultiAddActive)
    bpy.utils.register_class(WIGGLE_OT_CMultiAddPicked)
    bpy.utils.register_class(WIGGLE_OT_CMultiRemove)
    bpy.utils.register_class(WIGGLE_OT_CMultiClear)
    bpy.types.PoseBone.wiggle_collider_pick = bpy.props.PointerProperty(name='Pick', type=bpy.types.Object, poll=collider_poll, override={'LIBRARY_OVERRIDABLE'})
    bpy.types.PoseBone.wiggle_collider_pick_head = bpy.props.PointerProperty(name='Pick', type=bpy.types.Object, poll=collider_poll, override={'LIBRARY_OVERRIDABLE'})
    bpy.types.PoseBone.wiggle_collider_multi = bpy.props.CollectionProperty(type=WiggleColliderItem, override={'LIBRARY_OVERRIDABLE','USE_INSERTION'})
    bpy.types.PoseBone.wiggle_collider_multi_index = bpy.props.IntProperty(default=0, override={'LIBRARY_OVERRIDABLE'})
    bpy.types.PoseBone.wiggle_collider_multi_head = bpy.props.CollectionProperty(type=WiggleColliderItem, override={'LIBRARY_OVERRIDABLE','USE_INSERTION'})
    bpy.types.PoseBone.wiggle_collider_multi_index_head = bpy.props.IntProperty(default=0, override={'LIBRARY_OVERRIDABLE'})
...
#        update=lambda s, c: update_prop(s, c, 'wiggle_enable')
    ...
    bpy.types.PoseBone.wiggle_collider_type = bpy.props.EnumProperty(
        name='Collider Type',
        items=[('Object','Object','Collide with a selected mesh'),('Objects','Objects','Collide with multiple specific meshes'),('Collection','Collection','Collide with all meshes in selected collection')],
        override={'LIBRARY_OVERRIDABLE'},
    ...
    # LENGTH PRESERVATION / JAM HANDLING
    bpy.types.PoseBone.wiggle_preserve_length = bpy.props.BoolProperty(
        name='Preserve Length',
        description='Clamp stretch along bone to prevent scaling',
        default=True,
        override={'LIBRARY_OVERRIDABLE'},
        update=lambda s, c: update_prop(s, c, 'wiggle_preserve_length')
    )
    bpy.types.PoseBone.wiggle_max_stretch = bpy.props.FloatProperty(
        name='Max Stretch',
        description='Allowed extra stretch as a fraction of rest length before jam handling',
        min=0.0, max=2.0, default=0.15, subtype='FACTOR',
        override={'LIBRARY_OVERRIDABLE'},
        update=lambda s, c: update_prop(s, c, 'wiggle_max_stretch')
    )
    bpy.types.PoseBone.wiggle_jam_mode = bpy.props.EnumProperty(
        name='When Jammed',
        description='What to do when over-stretched by collisions/parent motion',
        items=[('HARD_STOP','Hard Stop','Clamp and damp velocity'),
               ('PASS_THROUGH','Pass Through','Temporarily bypass collision when overstretched')],
        default='HARD_STOP',
        override={'LIBRARY_OVERRIDABLE'},
        update=lambda s, c: update_prop(s, c, 'wiggle_jam_mode')
    )
#HEAD COLLISION
...
def unregister():
    bpy.utils.unregister_class(WIGGLE_OT_CMultiClear)
    bpy.utils.unregister_class(WIGGLE_OT_CMultiRemove)
    bpy.utils.unregister_class(WIGGLE_OT_CMultiAddPicked)
    bpy.utils.unregister_class(WIGGLE_OT_CMultiAddActive)
    bpy.utils.unregister_class(WIGGLE_UL_ColliderObjects)
    bpy.utils.unregister_class(WiggleColliderItem)
...
if __name__ == "__main__":
    ...
    bpy.types.PoseBone.wiggle_max_compress = bpy.props.FloatProperty(
        name='Max Compress',
        description='Allowed compression below rest length (fraction). 0 = never shrink',
        min=0.0, max=0.9, default=0.0, subtype='FACTOR',
        override={'LIBRARY_OVERRIDABLE'},
        update=lambda s, c: update_prop(s, c, 'wiggle_max_compress')
    )
    bpy.types.PoseBone.wiggle_pass_cooldown = bpy.props.IntProperty(
        name='Pass-Through Cooldown',
        description='Internal: frames to keep ignoring collision after a pass-through event',
        min=0, max=30, default=0, subtype='UNSIGNED',
        override={'LIBRARY_OVERRIDABLE'}
    )
